...
class Device {
  constructor (deviceInfo, websocket) {
    ...
    const index = websocket._info.split(' ')[0];
...
class DeviceManager extends Emitter {
  ...
  registerDevice (deviceInfo, websocket) {
    const existDevice = this.deviceList.filter(dvc => dvc.deviceId === deviceInfo.deviceId + '|' + deviceInfo.name)[0];
    if (existDevice) {
      ...
      const device = new Device(deviceInfo, websocket);
  ...
  getDevice (websocket) {
    const ret = this.deviceList.filter((dvc) => dvc.deviceId === websocket._deviceId && dvc.websocket === websocket);
    if (ret.length > 0) {
      return ret[0];
    }
    else {
